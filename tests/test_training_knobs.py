...
# Built by the `protocol_shards` fixture; these are gitignored artifacts, so a
# module-level constant pointing at them breaks on a fresh clone.
...
def _train(shards, tmp_path, **kwargs):
    return train(shards["train"], str(tmp_path), **TINY, steps=4, seed=17,
                 device="cpu", checkpoint_every=4, batch_size=2, **kwargs)


def test_grad_clip_actually_bounds_the_update(protocol_shards, tmp_path):
    """Clipped and unclipped runs must reach different weights."""
    loose = _train(protocol_shards, tmp_path / "loose", grad_clip=None)
    tight = _train(protocol_shards, tmp_path / "tight", grad_clip=1e-4)
    assert loose["final_loss"] != tight["final_loss"], (
        "an aggressive grad_clip changed nothing -- clipping is not wired in")


def test_grad_clip_is_recorded(protocol_shards, tmp_path):
    assert _train(protocol_shards, tmp_path / "r", grad_clip=0.5)["grad_clip"] == 0.5


def test_reported_grad_norm_is_pre_clip(protocol_shards, tmp_path):
    """The health gate needs the true norm, not the clipped-to value."""
    result = _train(protocol_shards, tmp_path / "n", grad_clip=1e-6)
...
def test_weight_decay_exempts_norms_and_biases(protocol_shards, tmp_path):
    ...
    try:
        _train(protocol_shards, tmp_path / "wd", weight_decay=0.1)
    finally:
...
def test_weight_decay_changes_results(protocol_shards, tmp_path):
    heavy = _train(protocol_shards, tmp_path / "h", weight_decay=0.5)
    none = _train(protocol_shards, tmp_path / "z", weight_decay=0.0)
    assert heavy["final_loss"] != none["final_loss"]


def test_eval_batch_size_does_not_move_accuracy(protocol_shards, tmp_path):
    small = _train(protocol_shards, tmp_path / "e1", heldout_shard=protocol_shards["heldout"], eval_batch_size=1)
    large = _train(protocol_shards, tmp_path / "e2", heldout_shard=protocol_shards["heldout"], eval_batch_size=64)
...
def test_resume_across_architectures_is_refused(protocol_shards, tmp_path):
    out = tmp_path / "arch"
    _train(protocol_shards, out, architecture="reference-v1")
    checkpoint = sorted(out.glob("checkpoint-*.pt"))[-1]
    with pytest.raises(ValueError, match="architecture"):
        train(protocol_shards["train"], str(out), **TINY, steps=8, seed=17, device="cpu",
              checkpoint_every=4, batch_size=2, resume=str(checkpoint),
              architecture="reex-v2")


def test_resume_records_and_checks_precision(protocol_shards, tmp_path):
    out = tmp_path / "prec"
    result = _train(protocol_shards, out)
...
def test_replay_within_tolerance_does_not_raise(protocol_shards, tmp_path):
...
def test_replay_outside_tolerance_still_raises(protocol_shards, tmp_path):
    ledger = tmp_path / "l.jsonl"
    _append_or_verify_replay(str(ledger), _ledger_entry())
